...
import { GoalsEditor, MacroTargetsEditor, DietaryPreferencesEditor, NotificationsEditor, dietTypeLabels, allergyLabels } from "./ProfileEditors";
...
export function Modal({ type, close, addWater, setWaterTotal, saveWaterGoal, water, waterGoal, waterDate, next, notify, setTab, onPhoto, uploadedPhoto, uploadedData, analysis, analyzing, analysisError, onAnalyze, onAddAnalysis, profile, target, macroTargets, onLogout, loggingOut, savedProducts, onSaveProducts, onSaveProfileGoals, onSaveProfileMacros, onSaveProfileDietary, onSaveProfileNotifications, weightLogs, onSaveWeight, onDeleteWeight, manualStartMode, manualInitialFood, recentFoods, onAddManualFood }: any) {
  ...
  return <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && close()}><section className={`modal-sheet ${type === "result" ? "result-sheet" : ""}`}>
    ...
    {type === "profile" && <><div className="profile-head"><div className="avatar big">{profileInitials(profile?.name)}</div><div><h2>{profile?.name || "Your profile"}</h2><p>{profileGoalLabel(profile?.primary_goal)} · {profile?.weight_unit === "lb" ? "Imperial weight" : "Metric weight"}</p></div></div><div className="modal-list settings"><button onClick={() => next("weight")}><span><strong>Weight progress</strong><small>{weightLogs.length ? `${weightInUnit(weightLogs[weightLogs.length - 1].weight_kg, profile?.weight_unit || "kg").toFixed(1)} ${profile?.weight_unit || "kg"} · ${weightLogs.length} saved ${weightLogs.length === 1 ? "entry" : "entries"}` : "Log weight and review trends"}</small></span><b>›</b></button><button onClick={() => next("goals")}><span><strong>Goals & targets</strong><small>{Number(target || 0).toLocaleString()} kcal daily goal</small></span><b>›</b></button><button onClick={() => next("macros")}><span><strong>Macro targets</strong><small>{macroTargets.protein}g protein · {macroTargets.carbs}g carbs · {macroTargets.fat}g fat</small></span><b>›</b></button><button onClick={() => next("dietary")}><span><strong>Dietary preferences</strong><small>{profile?.diet_type ? `${dietTypeLabels[profile.diet_type]} diet` : "No diet set"} · {(profile?.allergies?.length || 0) === 0 ? "No declared allergies" : `${profile.allergies.length} allerg${profile.allergies.length === 1 ? "y" : "ies"}: ${profile.allergies.map((a: string) => allergyLabels[a] || a).join(", ")}`}</small></span><b>›</b></button><button onClick={() => next("notifications")}><span><strong>Notifications</strong><small>{[profile?.notification_prefs?.meals && "meals", profile?.notification_prefs?.water && "water", profile?.notification_prefs?.weekly && "weekly"].filter(Boolean).length ? `${[profile?.notification_prefs?.meals && "meals", profile?.notification_prefs?.water && "water", profile?.notification_prefs?.weekly && "weekly"].filter(Boolean).join(", ")} reminders on` : "All reminders off"}</small></span><b>›</b></button><button><span><strong>Subscription</strong><small>MealRoute account</small></span><b>›</b></button><button><span><strong>Privacy & your data</strong><small>Export or delete account</small></span><b>›</b></button></div><button className="text-button danger" disabled={loggingOut} onClick={onLogout}>{loggingOut ? "Logging out…" : "Log out"}</button></>}
    ...
    {type === "dietary" && profile && <DietaryPreferencesEditor profile={profile} onBack={() => next("profile")} onSave={onSaveProfileDietary} />}
    {type === "notifications" && profile && <NotificationsEditor profile={profile} onBack={() => next("profile")} onSave={onSaveProfileNotifications} />}
